...
## Correction R2 — source-authority provenance

Correction cycle 2 preserves every R1 runtime, validation, subject-lineage and
consumer behavior while removing `compiledArtifactSources`. The six established
Instagram B4 YAML artifacts now exist as committed authority documents under
`intelligence/engines/instagram_intelligence/branches/content_behavior`. They
were normally published in intermediate authority commit
`c98b4edfa64b6711d290947ea61236573e029dd2`, tree
`15be1c29422b0079acce3c9220e0413eab750df8`.

The backend generator now reads every Instagram artifact exclusively with the
equivalent of `git show <exact-sha>:<exact-path>`. The explicit
`independentAuthorityCommit` flag applies only to the registered Instagram
processor. It requires an exact existing 40-character commit and exact safe
paths while leaving ancestor enforcement unchanged for the ten existing
Brand/Product bundles. Every generated artifact's Git blob matched its claimed
authority blob byte-for-byte. The manifest records the intermediate authority
SHA and has canonical bundle hash
`dfa31f3f85f2c847a5075b632f5dfad5e822615658cb0db948c4b9b8b86a6154`.
Existing Brand/Product generated bytes remain unchanged.

The complete generator test file passed 9/9. It proves processor lookup without
positional assumptions, independent-line byte reproduction, artifact and bundle
hash changes after an authority byte change, rejection of missing/wrong paths,
invalid commits, dirty sources, non-frozen/invalid semantic ownership, and
verify-only drift, plus deterministic generation and resistance to backend-only
string substitution. Contract integrity, runtime registry, structural/semantic
validation, B4 contract, and architecture tests passed 4 files / 33 tests. The
PostgreSQL/runtime regression matrix passed 7 files / 41 tests, including B4
vertical/replay/failure-current, exact subject exclusion, tenant/account/
authorization-generation fencing, and B3A/B1 regressions.

All 88 migrations applied cleanly and status was current; migration 88 remained
byte-identical at
`d02b75e50f883150c9d14908c82ef10d9b0de4c2b464168567634dd0157009e4`.
Prisma generate/validate, backend build, scoped ESLint, diff/secret/forbidden-
artifact checks, frontend typecheck/build, and the authenticated 1440px browser
replay passed. The browser returned consumer HTTP 200 with no overflow, console
or page errors, and no live provider/model request.

The R2 backend correction was normally pushed and independently fetched back:
local/remote SHA `4ebe7d95f33f14d3e650bcfa54a117bfd56373e1`, local/remote tree
`97791d71bbced9b7eedd44f9af80df4e31353738`. Frontend remains unchanged at
`bc8523183ae4892c0b538474d2f5444a1ea356d8` / `7f3016c01500083ae2f536ebd0540219c9228dd7`.
B4 remains `EVIDENCE_READY_FOR_CHILD_SA_REVIEW`; `B4_ACCEPTED = NO`; B3B remains
prohibited.
