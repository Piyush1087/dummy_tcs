# Creator Centre — Commercial Setup / Rate Card V0 Product Decision Register V2

`CREATOR_COMMERCIAL_SETUP_RATE_CARD_V0_PRODUCT_DECISION_REGISTER_V2`

Date: 2026-09-15  
Status: **PRODUCT_FROZEN_FOR_V0**  
Supersedes: `CREATOR_COMMERCIAL_SETUP_RATE_CARD_V0_PRODUCT_DECISION_REGISTER_V1`  
Implementation: **NOT_AUTHORIZED_BY_THIS DOCUMENT ALONE**

V2 changes only:

1. Reel reference duration is `<15 seconds`.
2. The Instagram-only Creator-facing paid-amplification label is `Partnership Ads`, not `Partnership / Spark Ads`.
3. All other V1 Product behavior remains unchanged.
...
Brand Brief usage-right concepts include:
...
For the Instagram-only MVP, the Creator-facing paid-amplification label is:

```text
Partnership Ads
```

`Spark Ads` is not shown in the V0 Creator UI. Technical reconciliation may map `Partnership Ads` to an accepted shared internal concept without changing the Creator-facing label.
...
### 7.2 Partnership Ads authorization
...
6. **Rights are explicit.** Link-in-Bio, Partnership Ads authorization and content usage rights apply only when explicitly included in the final agreement and only for the agreed duration/scope.
...
7. Partnership Ads has one starting amount with the fixed V0 reference authorization duration of 15 days;
...
```text
CREATOR_RATE_CARD_V0_PRODUCT = FROZEN_V2
...
REEL_REFERENCE_DURATION = <15_SECONDS

CREATOR_FACING_PAID_AMPLIFICATION_LABEL = PARTNERSHIP_ADS

UGC_RIGHTS_AND_CURRENCY_BEHAVIOR = UNCHANGED_FROM_V1
