# Windows Host — Post-AWS ARM64 Build Capacity Cleanup Execution V4

`WINDOWS_POST_AWS_ARM64_BUILD_CAPACITY_CLEANUP_EXECUTION_V4`
...
The earlier 6.5 GB figure was an approximate estimate for recent ARM64 Docker artifacts, not a proven additional path or a promised recovery amount. The audit independently measured approximately 6.406 GB of layer-aware superseded ARM64 image-family data. Actual Docker-internal recovery and physical Windows recovery must be measured after exact deletion and VHDX compaction.
...
## 9. Wave C — BuildKit dependency mapping, strictly read-only

No BuildKit deletion is authorized in this execution.

The audit established that no exact BuildKit deletion allowlist exists and that the protected latest ARM64 build exists only through BuildKit history/state. Therefore:
...
4. Produce a proposed exact future BuildKit deletion allowlist only where independence is proven.
5. Record overlaps, uncertainties, and the estimated exclusive bytes for each future candidate.
6. Delete zero BuildKit history records.
7. Delete zero BuildKit cache records.
8. Delete zero BuildKit content blobs, manifests, configs, or layer descriptors.
9. Delete zero builder containers.
10. Delete zero builder volumes.
11. Do not run any builder/buildx prune command, even if Docker labels content reclaimable.
12. Reverify the protected build record and its descriptors after the read-only mapping.

The mapping is evidence for a possible separately authorized future action. It is not cleanup authority.
...
BUILDKIT_CACHE_RECORDS_REMOVED =
NONE

BUILDKIT_CONTENT_BLOBS_REMOVED =
NONE
...
- read-only BuildKit protected-versus-future-candidate dependency table;
