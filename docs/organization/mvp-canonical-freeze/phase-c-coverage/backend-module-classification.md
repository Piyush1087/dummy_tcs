...
**Evidence:** `src/app.module.ts` + `@Controller` declarations on freeze working tree  
**Freeze branch HEAD (audit):** `d684143b9dfad527c1b92d6a86fa4510296873d3`  
**RUN 1 hide SHA:** `13a1dedc0ead8eef24a27c48067364258119b0fc`
...
| `PrismaModule` `HealthModule` `S3Module` `ConfigModule` `ThrottlerModule` | INFRA | |
...
| `IntelligenceConsumerModule` | CANONICAL | PI in-process; **no HTTP controller** |
...
| `PublicBrandModule` | CANONICAL | `api/v1/public/brands/:slug` |
| `PublicCreatorModule` | DEFERRED | Live `GET …/public/creators/:slug/media-kit` (Centre media-kit). FE route redirects Home. |
...
| `CreatorHomeModule` / `GET /api/v1/creator/home` | ABSENT | Accepted C-02A pack not in freeze `AppModule`. Do not treat Centre as C-02A. |
| `CreatorCentreModule` | DEFERRED / OUT | Still imported (media-kit + analytics). Not C-02A Home. |

## HTTP controller prefixes (complete `@Controller` list)

| Controller prefix | Class |
| --- | --- |
| `health` `health/live` | INFRA |
| (root `AppController`) | INFRA |
| `api/v1/s3` | INFRA |
| `api/v1/auth` | CANONICAL |
| `api/v1/discovery` `api/v1/brand` `api/v1/brand/social-sync` | CANONICAL onboarding |
| `api/v1/brand/home` `api/v1/chat/conversations` | CANONICAL Chat Home |
| `api/v1/brand-centre` `api/v1/orchestration` | CANONICAL Centre / UCE bridge |
| `api/v1/brand-uce` | CANONICAL |
| `api/v1/campaign-opportunities` `api/v1/creator/campaigns` `api/v1/creator` (applications) `api/v1/creator-uce` | CANONICAL C-03 |
| `api/v1/collaboration` | CANONICAL Brand; Creator C-04 deferred |
| `api/v1/brand/settings` `api/v1/brand/team-invitations` `api/v1/pricing` `api/v1/escrow*` `api/v1/hardened-escrow` | CANONICAL |
| `api/v1/webhooks/subscription` `api/v1/webhooks/escrow*` | CANONICAL fail-closed |
...
Hiding the frontend does **not** unwire these APIs. Later freeze amendment may remove OUT modules from `AppModule`. Not done this freeze (Prisma drop forbidden).
