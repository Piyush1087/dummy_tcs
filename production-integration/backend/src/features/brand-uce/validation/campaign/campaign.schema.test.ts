...
describe("Campaign definition Phase 1D validation", () => {
  ...
  it("does not allow AI_RECOMMENDED through the persisted manual Draft boundary", () => {
    expect(
      createCampaignDraftInputSchema.safeParse({
  ...
  it("keeps KPI fields out of Brand-authored strategy input", () => {
    const parsed = campaignStep1CompletionSchema.parse({
      ...
      strategy: {
        ...validStrategy,
        primaryKpiId: "should-not-be-authoritative",
      },
    });
  ...
  it("requires 1-5 archetypes at step completion and age within 13-65", () => {
    ...
    expect(
      campaignStep2CompletionSchema.safeParse(validTargeting).success,
    ).toBe(true);
  });
  ...
  it("enforces canonical commercial rules", () => {
    expect(
      campaignStep3CompletionSchema.safeParse(validCommercials).success,
    ).toBe(true);
    expect(
      campaignStep3CompletionSchema.safeParse({
        ...validCommercials,
        totalCampaignBudget: 9_999,
      }).success,
    ).toBe(false);
    expect(
      campaignStep3CompletionSchema.safeParse({
  ...
  it("supports focused post-publish patches while leaving editability to CampaignService", () => {
    expect(
      updateLiveCampaignInputSchema.safeParse({
    ...
    expect(
      updateLiveCampaignInputSchema.safeParse({
        campaignId: "campaign-1",
        strategy: { campaignVisibility: "INVITE_ONLY" },
      }).success,
    ).toBe(true);
    expect(
      updateLiveCampaignInputSchema.safeParse({ campaignId: "campaign-1" })
        .success,
    ).toBe(false);
  });
});
