...
const validatePublishingWindow = (
  value: { publishingSchedule?: "EVERGREEN" | "SCHEDULED"; publishFrom?: Date | null; publishUntil?: Date | null },
  ctx: z.RefinementCtx,
) => {
  ...
  if (value.publishingSchedule === "SCHEDULED") {
    ...
    if (value.publishFrom && value.publishUntil && value.publishUntil < value.publishFrom) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["publishUntil"], message: "publishUntil must be on or after publishFrom." });
    }
  }
};

export const campaignStrategyInputSchema = z.object(campaignStrategyFields).superRefine(validatePublishingWindow);
export const campaignStrategyPatchSchema = z.object(campaignStrategyFields).partial().superRefine(validatePublishingWindow);
...
const validateTargeting = (
  value: { minimumFollowers?: number; maximumFollowers?: number | null; audienceAgeMin?: number; audienceAgeMax?: number },
  ctx: z.RefinementCtx,
) => {
  if (value.minimumFollowers != null && value.maximumFollowers != null && value.maximumFollowers <= value.minimumFollowers) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["maximumFollowers"], message: "maximumFollowers must be greater than minimumFollowers." });
  }
  if (value.audienceAgeMin != null && value.audienceAgeMax != null && value.audienceAgeMax < value.audienceAgeMin) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["audienceAgeMax"], message: "audienceAgeMax must be >= audienceAgeMin." });
  }
};

export const campaignTargetingInputSchema = z.object(campaignTargetingFields).superRefine(validateTargeting);
export const campaignTargetingPatchSchema = z.object(campaignTargetingFields).partial().superRefine(validateTargeting);
...
const validateCommercials = (
  ...
  if (value.receivesBrandSupport === false && (value.brandSupportType != null || value.brandSupportEstimatedValue != null)) {
  ...
  if (value.commercialOffer != null && value.totalCampaignBudget != null && value.totalCampaignBudget < value.commercialOffer) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["totalCampaignBudget"], message: "totalCampaignBudget must be >= commercialOffer." });
  }
};

export const campaignCommercialsInputSchema = z.object(campaignCommercialFields).superRefine(validateCommercials);
export const campaignCommercialsPatchSchema = z.object(campaignCommercialFields).partial().superRefine(validateCommercials);
...
export const campaignStep2CompletionSchema = campaignTargetingInputSchema.superRefine((value, ctx) => {
  if (value.creatorArchetypes.length < 1) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["creatorArchetypes"], message: "At least one creator archetype is required." });
  }
});
...
export const updatePostPublishCampaignInputSchema = updateLiveCampaignInputSchema;
