...
export function createIdentityRuntime(deps: IdentityRuntimeDependencies) {
 const runtime:CompilerRuntime={
  ...
  async runDeterministicTask({executionId,task,request,canonicalDependencies,persistResults}):Promise<TaskResult>{
   const processorExecutionId=await deps.telemetry.taskStarted(executionId,task);try{let values:Record<string,unknown>;if(task.processorId==="reporting_currency"){const country=canonicalDependencies.country;if(typeof country!=="string")throw new Error("country is required for reporting_currency");values={reporting_currency:country==="IN"?"INR":"USD"};}else throw new Error(`No deterministic resolver configured for ${task.processorId}`);await deps.persistence.persist({task,entityId:request.entityId,values,persistResults});const result:TaskResult={taskId:task.id,state:"SUCCEEDED",values};await deps.telemetry.taskFinished(processorExecutionId,result);return result;}catch(error){const result:TaskResult={taskId:task.id,state:"FAILED_PRECHECK",error:{code:"DETERMINISTIC_TASK_FAILED",message:error instanceof Error?error.message:"Deterministic task failed"}};await deps.telemetry.taskFinished(processorExecutionId,result);return result;}
  }
 };
