...
export function App() {
  ...
  const handleAction = async (action: CampaignUiAction): Promise<CampaignUiActionResult> => {
    ...
    if (result.ok) setView(await adapter.getCampaignPage(action.campaignId));
    return result;
  };
  ...
  return (
    <ApplicationShell><label className="scenarioSelector">Staging scenario <select onChange={(event) => setScenario(event.target.value as ScenarioId | "normal")} value={scenario}><option value="normal">Normal executable read</option>{scenarioIds.map((id) => <option key={id} value={id}>{scenarioLabel[id]}</option>)}</select></label>
      {view ? <CampaignPage adapter={adapter} onAction={handleAction} view={view} /> : <p className="loadingState">Loading Campaign content area…</p>}
    </ApplicationShell>
  );
}
